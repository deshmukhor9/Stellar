...
readStatus: '',
  ...
  const handleStatusToggle = (status:any) => {
    ...
    if (status === 'completed') {
      setShowDatePicker(true); // Show date picker if completed status is selected
    } else {
      setShowDatePicker(false);
      setFormData({ ...formData, completionDate: null }); // Clear completion date if status changes from completed
    }
  };
  ...
  const formattedDate = formData.completionDate instanceof Date
  ...
  const handleSubmit = async () => {
    try {
        ...
        const serialNumberString = String(formData.serialNumber).trim();
        // console.log("Serial Number (to be searched):", serialNumberString);

        // Check if a matching serial number exists in the adminentries collection
        const querySnapshot = await getDocs(
            query(collection(db, 'adminentries'), where('serialNumber', '==', serialNumberString))
        );
        // console.log("Query Snapshot Size:", querySnapshot.size); // Debugging line


        let updateData = { ...formData }; // Start with the form data

        if (!querySnapshot.empty) {
            // If a document with the serial number exists, update the existing data
            const docRef = querySnapshot.docs[0].ref; // Get the document reference
            await updateDoc(docRef, updateData);
            alert('Entry updated successfully!'); // Show update success message
        } else {
            // If the document does not exist, add it as a new entry
            await addDoc(collection(db, 'adminentries'), updateData); // Corrected line
            alert('Entry added successfully!'); // Show add success message
        }
  ...
  const handleFetchData = async () => {
    try {
      ...
      const q = query(collection(db, 'adminentries'), where('serialNumber', '!=', null)); // '!=' ensures non-null serialNumbers
  ...
  const handleYes = () => {
    if (selectedData) {
      const currentStatus = selectedData.status || 'inProgress'; // Default to 'inProgress' if status is missing
...
<View style={styles.toggleContainer}>
      ...
      />
        ...
        <Button title="Update Job" onPress={handleSubmit} />
...
<Modal animationType="slide" transparent={true} visible={showQueryModal} onRequestClose={() => setShowQueryModal(false)}>
          <View style={styles.fetchOverlay}>
            <View style={styles.fetchContent}>
              <Text style={styles.fetchTitle}>Select a Job</Text>
              <ScrollView contentContainerStyle={styles.scrollContent}>
                {queryData.map((item, index) => (
                  <TouchableOpacity key={index} onPress={() => handleDataBlockClick(item)} style={styles.dataBlock}>
                    ...
                    <Text style={styles.fetchText}>Job Status: {item.status}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
